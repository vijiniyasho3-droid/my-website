...
const TIME_LIMIT = 45; // 45 seconds for medium mode
const WIN_PUZZLES = 4; // Win after 4 puzzles completed correctly
...
async function handleSubmit() {
  ...
  if (isCorrect) {
    ...
    if (puzzlesSolved >= WIN_PUZZLES) {
      ...
      setTimeout(() => {
        window.location.href = `win.html?score=${score}&timeTaken=${timeTaken}&difficulty=medium&puzzlesSolved=${puzzlesSolved}`;
      }, 1500);
      return;
    }
...
function endGame() {
  ...
  const url = `result.html?score=${score}&timeTaken=${timeTaken}&difficulty=medium&correct=${lastAnswerCorrect}&puzzlesSolved=${puzzlesSolved}`;
