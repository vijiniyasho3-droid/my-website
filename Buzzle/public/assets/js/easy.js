...
const TIME_LIMIT = 60; // 60 seconds for easy mode
const WIN_PUZZLES = 5; // Win after 5 puzzles completed correctly
...
async function handleSubmit() {
  ...
  if (isCorrect) {
    ...
    if (puzzlesSolved >= WIN_PUZZLES) {
      ...
      setTimeout(() => {
        window.location.href = `win.html?score=${score}&timeTaken=${timeTaken}&difficulty=easy&puzzlesSolved=${puzzlesSolved}`;
      }, 1500);
      return;
    }
...
function endGame() {
  ...
  const url = `result.html?score=${score}&timeTaken=${timeTaken}&difficulty=easy&correct=${lastAnswerCorrect}&puzzlesSolved=${puzzlesSolved}`;
